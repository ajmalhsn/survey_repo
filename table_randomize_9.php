...
$sql2="SELECT id,rotation_count_9 FROM rotation_plan_primary ORDER BY rotation_count_9 ASC LIMIT 1";
$result2=$link->query($sql2);
    if($result2->num_rows > 0){
        while($row=$result2->fetch_assoc()){
            $id=$row["id"];
            $r=$row["rotation_count_9"];
        }
    }
$r++;

$sql3="UPDATE rotation_plan_primary SET rotation_count_9=$r WHERE id=$id";
if($link->query($sql3)){


}

$sql="SELECT id,attributes FROM rotation_plan r,attribute_q9 a WHERE r.stid = a.id AND rid=$id";
...
$content.='</tbody></table>';
echo '<br>';
echo '<p>Q9.I would like to understand your management style at work. This means people management and dealing with employees. We would like to understand to what extent do you agree with these statements for yourself.</p>';
